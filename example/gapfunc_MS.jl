...
function G02wrapped(fdlr, kgrid, param)
    ...
    for (ki, k) in enumerate(kgrid)
        for (ni, ωn) in enumerate(fdlr.ωn)
            ω = k^2 / 2 / me - μ
            green_dyn[ki, ni] = 1 / (ωn^2 + ω^2)
        end
    end
...
function G2wrapped(fdlr, kgrid, param, Σ::GreenFunc.Green2DLR)
    ...
    for (ki, k) in enumerate(kgrid)
        for (ni, ωn) in enumerate(fdlr.ωn)
            ...
            green_dyn[ki, ni] = 1 / ((ωn - ΣI)^2 + (ω + ΣR)^2)
        end
    end
    return green_dyn
end
...
function deltaFinit(fdlr, kgrid, param)
    @unpack me, μ = param

    Ω_c = freq_sep
    F = zeros(Float64, (kgrid.size, fdlr.size))
    delta = ones(Float64, (length(kgrid.grid), fdlr.size))
    delta0 = zeros(Float64, kgrid.size)
    for (ki, k) in enumerate(kgrid.grid)
        for (ni, ωn) in enumerate(fdlr.ωn)
            e = k^2 / 2 / me - μ
            delta[ki, ni] = (1.0 - 2.0 * ωn^2 / (ωn^2 + Ω_c^2)) / (Ω_c^2 + e^2)
        end
    end

    return real(Lehmann.matfreq2tau(fdlr, delta, fdlr.τ; axis=2)), delta0, F
end
...
function gapIteration_Renorm(param, fdlr, kgrid, qgrids, kernel, kernel_bare, G2;
    Nstep=1e2, rtol=1e-6, shift=2.0)
    @unpack dim = param

    Δ, Δ0, F = deltaFinit(fdlr, kgrid, param)

    delta = zeros(Float64, (kgrid.size, fdlr.size))
    delta0 = zeros(Float64, (kgrid.size))

    Δms = 0.0
    dΔ = zeros(Float64, (kgrid.size, fdlr.size))
    kF_label = searchsortedfirst(kgrid.grid, param.kF)
    ...
    while (n < Nstep && err > rtol)
        ...
        Δ_freq = real(Lehmann.tau2matfreq(fdlr, Δ, fdlr.n; axis=2))
        Δms = Δ_freq[kF_label, 1] + Δ0[kF_label]
        dΔ = Δ_freq - Δ_freq[kF_label, 1] * ones(Float64, (kgrid.size, fdlr.size))
        # delta0 =
...
function gapIteration(param, fdlr, kgrid, qgrids, kernel, kernel_bare, G2;
    ...
    dΔ = zeros(Float64, (kgrid.size, fdlr.size))
    kF_label = searchsortedfirst(kgrid.grid, param.kF)
    # n0_label = searchsortedfirst(fdlr.n, 0)
...
function gapfunction(beta, rs, channel::Int, dim::Int, sigmatype)
    ...
    lamu, Δ, Δ0, F = gapIteration(param, fdlr, kgrid, qgrids, kernel, kernel_bare, G2; shift=2.0)
    # lamu, Δ, Δ0, F = gapIteration(param, fdlr, kgrid, qgrids, kernel_freq, kernel_bare, G2; shift=1.0)
